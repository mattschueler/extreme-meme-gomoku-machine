...
"""
DYNAMIC_SKELETONS = """
int get__{BITARRAY_ID}(int __y, int __x);
void set__{BITARRAY_ID}(int __y, int __x, int value);
"""
...
def preprocess(inputfile, outputfile):
	...
	def replace_set(variable_name, line):
	    ...
	    context = line[0:line.index(variable_name)]
	    return context + "set__{BITARRAY_ID}({x},{y},{value})".format(**{"BITARRAY_ID":variable_name,"x":x,"y":y,"value":value})
	...
	FINAL_DYNAMIC_CODE = ""
	FINAL_DYNAMIC_SKELETONS = ""
	for x in bitarrays:
	    FINAL_DYNAMIC_SKELETONS = DYNAMIC_SKELETONS.format(**bitarrays[x])
	    FINAL_DYNAMIC_CODE = FINAL_DYNAMIC_CODE + DYNAMIC_CODE_FOR_INSERTION.format(**bitarrays[x])

	FINAL_CODE = COMMON_CODE_FOR_INSERTION + FINAL_DYNAMIC_SKELETONS + CODE + FINAL_DYNAMIC_CODE
